...
| `pop_sandbox.py new <project> <agent>` | Proposes a local interactive coding docker and writes its profile only after exact hash confirmation. `--check-fresh` checks a generated profile; `list` inventories local profiles; `rm` removes already-cleaned generated artifacts. Unavailable from installed harnesses. |
