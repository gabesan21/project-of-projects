...
| `pop_install_included.py` | Installs/**updates** the harness declared in `_templates/included-manifest.json`: mirrors the managed set into the target, **prunes** whatever left the source and stamps the source harness's `content_sha` into it. `--check-fresh <dir>` recomputes it and fails closed when the target has fallen behind (`pop_validate` reports it as a violation); `--sha` prints the source's version; `--check` only checks whether it is installed; `--audit-manifest` audits manifest closure. |
