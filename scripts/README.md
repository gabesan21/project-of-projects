...
| `pop_worktree.py` | Resolves route and manages task worktrees; a local scope refuses them, external yolo starts from `develop`. |
...
```
python3 scripts/pop_task.py my-project 1.1.1-user-table-creation --title "User table"
python3 scripts/pop_task.py my-project M-1.1-adjust-contract --title "Adjust contract"
python3 scripts/pop_move.py M-1.1-adjust-contract 002_planning --reason "planning started"
```

Run `python3 -m unittest discover -s scripts/tests -v`, `python3 scripts/pop_install_unirepo.py --audit-manifest`, and `python3 scripts/pop_validate.py` before delivery.
