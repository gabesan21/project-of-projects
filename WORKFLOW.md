...
- **One judge, one artifact.** The judgment is born in `<id>.verify.md` ([[_templates/TASK-VERIFY|TASK-VERIFY]], ≤80 lines): the original request first, then criteria, specs and diff; findings with severity and evidence filtered by the skill's **materiality test**; a single verdict. A new round appends a section to the same file and **never** deletes the previous one; the highest-numbered one decides.
