...
- **Three classes of file, three routes — and only one of them is the kanban.** The class decides how it gets fixed; none of them is decided by a card's label:
  ...
  3. **Content** (code, manuscript, the real work): **only** through a task that legitimately reached `004_processing`. It is this class — and no other — that rule 13 protects.
