...
- **The merge belongs to the human:** the agent never merges a task PR on its own — only when commanded in the merge round. The human may command a merge at any moment, including **outside 006** and to test the deliverable (see "An explicit human command beats the flow"). Exception: a yolo task, whose PR targets `develop` and is merged by the critic; the final `develop` → PR-branch PR still belongs to the human (Yolo mode section).
