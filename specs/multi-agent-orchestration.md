...
- 002 always uses a separate planner; the `005_closing` gate is judged by **exactly one judge per round**: the **Judge Dredd** ([[specs/judge-dredd|contract]]), in a fresh context, tier per the WORKFLOW matrix (medium in `S`/`M`, strong in `L`/`critical`) — `size`/`critical` change tier and depth, never the number of judges. He judges by reading (diff + recorded evidence), without re-running criteria or executing tests: a test is a `verify: phase` criterion and runs in the phase's verification task. Outside yolo there is no agentic reviewer: the gate is the human PR, and with no PR (root local PoP) there is no verification gate.
...
- [x] The Judge Dredd compares objective/specs with the diff and the recorded evidence, and separates plan defect from execution failure; returns carry a classified delta, re-entry is partial, and re-review is differential over the delta.
- [x] `pop_yolo.py`, `pop_move.py`, `pop_status.py`, and `test_yolo_flow.py` enforce scheduler, circuit, telemetry, and delivery behavior.
- [x] [[.agents/skills/advance-task/SKILL|advance-task]] and [[.agents/skills/judge-dredd/SKILL|judge-dredd]] define role boundaries; the act-1 powers, the `.verify.md` cap and phase verification live in [[specs/judge-dredd|Judge Dredd]].
