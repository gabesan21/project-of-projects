...
- `005_closing` gathers quality gate, delivery, and close-out: in yolo the gate has two exclusive configurations — **A**, the advocate + judge pair when `size: L` or `critical: true`; **B**, the single independent reviewer otherwise (trigger and routes in [[specs/adversarial-gate|adversarial gate]], *consult it when deciding which configuration applies to a yolo task*) — and outside yolo the gate is the human PR; with no PR (root local PoP) there is no verification gate. Close-out is mechanical and idempotent — it validates each precondition, skips completed effects, and repeats without duplicating integration, memory, cleanup, or PR.
