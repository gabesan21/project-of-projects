...
- `005_closing` gathers quality gate, delivery, and close-out: in yolo the gate is the **Judge Dredd**, a single judge at any `size` (medium tier in `S`/`M`, strong in `L`/`critical` — contract in [[specs/judge-dredd|Judge Dredd]], *consult it when judging act 1 of a yolo task*), and outside yolo the gate is the human PR; with no PR (root local PoP) there is no verification gate. Close-out is mechanical and idempotent — it validates each precondition, skips completed effects, and repeats without duplicating integration, memory, cleanup, or PR.
