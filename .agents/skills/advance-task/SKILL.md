...
- The `005_closing` judge runs in a clean session at the matrix tier (medium in configuration B, strong in A) and first checks whether the **original request** (the card's objective) was met — the brief is strategy, not an approved contract. It records `differential|full`, reason, surface, and tests; `full` is mandatory for critical tasks and after a `premissa` return (`scripts/pop_yolo.py verify-mode <id>` computes it from `return_kind`).
