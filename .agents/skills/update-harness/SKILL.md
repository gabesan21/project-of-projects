...
2. Compare: `python3 <source>/scripts/pop_install_unirepo.py --sha` against the `content_sha` field in `pop/.unirepo-harness.json`. Equal → harness is current, stop.
...
2. `python3 <source>/scripts/pop_install_unirepo.py .` — idempotent: overwrites only the managed set and prunes only what the previous inventory authorizes.
