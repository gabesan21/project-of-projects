...
- **Managed harness** (`WORKFLOW.md`, `_templates/`, `scripts/`, `.agents/skills/`) → **never edit it**. Staleness is resolved by reinstalling from the origin; the review performs the reinstall when that is the remedy, because it is mechanical and idempotent, and it **reports** any other finding in this class.
...
1. **Scripts first:** run `scripts/pop_status.py` (kanban overview, blocked tasks, pending gates — 003, review/human in `005_closing`, `awaiting_merge`, >14 days) and `scripts/pop_validate.py` (limits, frontmatter and warnings). INBOX.md is Dataview, not a source.
   **Harness version:** `python3 scripts/pop_install_unirepo.py --check-fresh .` reports the version installed here. Comparing it against the origin is the job of whoever installed it — it is not a finding of this review and it never justifies going looking for the origin.
