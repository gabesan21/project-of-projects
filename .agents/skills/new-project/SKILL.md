...
3. **Type `full-multi-repo`:** the parent folder's `pop/` stays **reduced** — only the general ROADMAP, **with no kanban** (no `kanban/`, `specs/` or `memory/`: tasks, specs and memory live in the repos) — and each repo, cloned at the root of the project folder (`categories/<category>/<project>/<repo>/`), gets its own `<repo>/pop/` via `python3 scripts/pop_install_included.py <repo>`, with `pop/worktrees/` in the **repo's** `.gitignore`. Each repo's AGENTS.md declares `type: included` + a **"Part of"** section with local links that don't prevent standalone use. The **project's** `.gitignore` ignores the clones; each repo gets its own ROADMAP (in its `pop/`) and the general ROADMAP points to them with a trigger.
