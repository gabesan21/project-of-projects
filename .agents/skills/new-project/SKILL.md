...
1. Create the structure — anatomy: the "Anatomy of a project" section of [[AGENTS|AGENTS]]; variations per type: [[TYPES|TYPES]]. This skill's specifics: the project's AGENTS.md from `_templates/AGENTS-PROJECT.md` (type, repos, PR branch, language, i18n) + CLAUDE.md symlink; real copies of the root core skills in `.agents/skills/` (new-task, advance-task, plan-roadmap, write-spec, sync-specs); the entire harness inside `pop/` — PROJECT.md/ROADMAP.md from the templates, filled with steps 1–4; optional RESEARCHES.md (`_templates/RESEARCHES.md`); the 6 empty `pop/kanban/` folders. The project's content lives **directly at the root** of the folder (there is no `project/`; the vault-root meta-project and not-yet-migrated projects: harness at the root, no `pop/`).
2. **Type `included` with a repo:** the project's root **is the repo clone itself** (`categories/<category>/<project>/`); run `python3 scripts/pop_install_included.py <repo>` to install or update the standalone `pop/` committed in the repo (with WORKFLOW.md, TYPES.md, INBOX.md, `_templates/` and `scripts/` inside `pop/`); add the clone's path to the PoP **root** `.gitignore`. Never create tool-specific folders (`.claude/` etc.) — the vault is agent-agnostic.
3. **Type `full-multi-repo`:** the parent folder's `pop/` stays **reduced** — only the general ROADMAP and the kanban for cross-repo tasks (no `specs/` and no `memory/`) — and each repo, cloned at the root of the project folder (`categories/<category>/<project>/<repo>/`), gets its own `<repo>/pop/` via `python3 scripts/pop_install_included.py <repo>`, with `pop/worktrees/` in the **repo's** `.gitignore`. Each repo's AGENTS.md declares `type: included` + a **"Part of"** section with local links that don't prevent standalone use. The **project's** `.gitignore` ignores `pop/worktrees/` and the clones; each repo gets its own ROADMAP (in its `pop/`) and the general ROADMAP points to them with a trigger.
