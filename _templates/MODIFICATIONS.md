...
> Completed rows stay with status `completed` (like epochs in the ROADMAP); it is the **task** rows that leave the modification files after valid memory (rule 17 of [[AGENTS|AGENTS]]). `M-<n>` ids are never reused.
