...
> File `pop/open_questions/YYYY-MM-DD-<slug>.md` (at the vault root, outside a project: `open_questions/`, without the `pop/` prefix), created by the **agent** when a decision depends on the human and belongs to no card. The `status: open` ones show up in the [[INBOX|INBOX]]. Blockquotes are instructions — delete them when filling it in.
