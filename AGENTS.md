...
| `yolo-critic` | Strong yolo critic in configuration B (non-critical `S`/`M`): single gate in `005_closing` (003 only for `critical`); two returns per gate and circuit breaker on the third — [[WORKFLOW\|WORKFLOW]]. |
...
| `optimize-memory` | Convert legacy memory into date folder + ledger + entries, and trim whatever exceeds the cap — without merging tasks or losing chronology, commits/PRs or critical decisions. |
