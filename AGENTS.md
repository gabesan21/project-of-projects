...
This vault is the **single source** of the harness. Every project that runs standalone — an `included` clone, an embedded repo of a `full-multi-repo` — never evolves WORKFLOW, templates, scripts or core skills on its own: it receives a **managed copy** via `python3 scripts/pop_install_included.py <dir>`, which mirrors the set declared in [[_templates/included-manifest.json|included-manifest]]. Each install writes into the target's `pop/.included-harness.json` both the source harness's `content_sha` and the **inventory** of what it wrote: the stamp makes "up to date" verifiable (`--check-fresh <dir>` fails closed when the target fell behind, and `pop_validate` reports staleness as a violation), and the inventory authorizes the next update's pruning — only what the installer brought before may be removed, because **a managed folder is not an exclusive folder** and the project keeps files of its own in `pop/scripts/`. Only the source answers about freshness: run from the installed copy, `--check-fresh` refuses. Fixing harness by editing the local copy is always wrong — the project **reinstalls**.
...
13. **Every change to the project goes through the kanban:** agents **never** touch the project content — nothing outside `pop/`, `.agents/` and `AGENTS.md` (in legacy: `project/` and the external repository) — outside a task that legitimately reached `004_processing`: through approval in 003 or, for non-critical yolo, through the 002→004 route. A change request with no card triggers `new-task` → `advance-task`; “apply”, “execute”, “urgent”, “finish it” or “in yolo” do not authorize a direct change.
