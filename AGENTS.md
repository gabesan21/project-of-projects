...
13. **Every change to the content goes through the kanban — harness never does:** agents **never** touch the project content — nothing outside `pop/`, `.agents/` and `AGENTS.md` (in legacy: `project/` and the external repository) — outside a task that legitimately reached `004_processing`: through approval in 003 or, for non-critical yolo, through the 002→004 route. A **content** change request with no card triggers `new-task` → `advance-task`; “apply”, “execute”, “urgent”, “finish it” or “in yolo” do not authorize a direct change. What sits **inside** that boundary is the rule's affirmative, not a loophole: harness is what the kanban consults, not what it gates — **harness maintenance is done directly, with no card, no task branch/worktree/PR** (the three classes of file are in the "Current scope" section of [[WORKFLOW|WORKFLOW]]). Creating a task to slim an AGENTS.md, fix a spec, a note or memory is a routing error, not diligence.
